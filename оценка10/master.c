...
static sem_t *sem_observer = NULL;
...
static void cleanup_master(void) {
    ...
    if (sem_observer) {
        sem_close(sem_observer);
        sem_unlink(SEM_OBSERVER_MUTEX);
        sem_observer = NULL;
    }
    ...
    // Очистка каталога наблюдателей
    DIR *dir = opendir(OBSERVERS_DIR);
    if (dir) {
        struct dirent *entry;
        while ((entry = readdir(dir)) != NULL) {
            if (strstr(entry->d_name, "observer_") != NULL) {
                char path[512];
                snprintf(path, sizeof(path), "%s/%s", OBSERVERS_DIR, entry->d_name);
                unlink(path);
            }
        }
        closedir(dir);
...
// Функция для отправки форматированной строки всем наблюдателям
void send_to_observers(const char *fmt, ...) {
    ...
    // Сканируем каталог на наличие FIFO наблюдателей
    DIR *dir = opendir(OBSERVERS_DIR);
    if (!dir) return;
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (strstr(entry->d_name, "observer_") != NULL && 
            strstr(entry->d_name, ".fifo") != NULL) {
            
            char fifo_path[512];
            snprintf(fifo_path, sizeof(fifo_path), "%s/%s", OBSERVERS_DIR, entry->d_name);
...
static void wait_for_registration(void) {
    ...
    send_to_observers("[OBSERVER] Все игроки зарегистрировались. Начинаем турнира\n");
}
...
static void run_tournament(void) {
    ...
    send_to_observers("[OBSERVER] Турнир начался! Всего матчей: %d\n", shared->total_matches);
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = i + 1; j < N; ++j) {
            ...
            send_to_observers("--- Матч %d/%d: Игрок %d vs Игрок %d ---\n",
                             shared->matches_played, shared->total_matches, i, j);
            ...
            if (res == 0) {
                ...
                shared->scores[i] += 1;
                shared->scores[j] += 1;
                send_to_observers("  Ничья! Оба игрока получают по 1 очку\n");
            } else if (res == 1) {
                printf("  Победил %d (+2)\n", i);
                shared->scores[i] += 2;
                send_to_observers("  Победил Игрок %d (+2 очка)\n", i);
            } else {
                printf("  Победил %d (+2)\n", j);
                shared->scores[j] += 2;
                send_to_observers("  Победил Игрок %d (+2 очка)\n", j);
...
static void print_final_results(void) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        send_to_observers("%d место: Игрок %d - %d очков\n",
               i + 1, results[i].id, results[i].score);
    }
...
int main(int argc, char *argv[]) {
    ...
    unlink_if_exists(SEM_OBSERVER_MUTEX);
    for (int i = 0; i < N; ++i) {
        char sname[64];
    ...
    sem_observer = sem_open(SEM_OBSERVER_MUTEX, O_CREAT | O_EXCL, 0600, 1);
    if (sem_observer == SEM_FAILED) {
        perror("sem_open observer");
        cleanup_master();
        return 1;
    }
    ...
    send_to_observers("[OBSERVER] Турнир завершен\n");
    sleep(1);
